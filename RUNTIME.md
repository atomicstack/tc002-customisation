...
way to a white canvas naming the effect in black and coming back with the paired
...
and it is not one of the mqtt command topics. `tc002 reboot` is the one-liner. it is also how `tc002-update.sh` clears a clock whose adbd has run
...
```bash
tc002 -s <ip> --token-file tokens reboot
```
...
| `GET` | `/screen` | `screen` | | `{"width":52,"height":16,"epoch","revision","brightness","power","rgb_base64":"…"}`: the frame as shown, after fades, and `brightness` is the level it is shown at (mid-ramp, not yet the target `/status` reports), before brightness. `?format=raw` returns the 2,496 rgb bytes as `application/octet-stream` |
...
so `--one` refuses to select one silently. `tc002-update.sh --device` accepts bare ips
...
tc002 dismiss door   # promotes parcel if door is active
tc002 dismiss        # dismisses only the current notification
