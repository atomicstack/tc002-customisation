...
const Tag = enum(u64) { timer = 1, supervisor = 2, listener = 3, mqtt = 4, mdns = 5, conn_base = 16 };
...
const Netd = struct {
    ...
    /// the clock style as `{"font","colour_mode","colour","colour2","gradient","spread","digits","fade"}`.
    fn clockJson(o: *Out, s: messages.ClockStyle) void {
        o.fmt("{{\"font\":\"{s}\",\"colour_mode\":\"{s}\",\"colour\":\"{x:0>2}{x:0>2}{x:0>2}\",\"colour2\":\"{x:0>2}{x:0>2}{x:0>2}\",\"gradient\":\"{s}\",\"spread\":{d},\"digits\":\"{s}\",\"fade\":{}}}", .{ enumName(clock.Font, s.font), enumName(clock.ColourMode, s.mode), s.colour[0], s.colour[1], s.colour[2], s.colour2[0], s.colour2[1], s.colour2[2], enumName(clock.Gradient, s.gradient), s.spread, enumName(clock.DigitStyle, s.digit), s.fade != 0 });
    }
